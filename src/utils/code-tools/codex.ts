...
export async function configureCodexApi(options?: CodexFullInitOptions): Promise<void> {
  ...
  if (!apiKey) {
    const { promptedApiKey } = await inquirer.prompt<{ promptedApiKey: string }>([
      {
        type: 'input',
        name: 'promptedApiKey',
  ...
  const configData: CodexConfigData = {
    model: 'gpt-4o', // Set the default model
    modelProvider: providerId, // Set our hardcoded provider as the default
    providers: [provider], // Only include our hardcoded provider
    mcpServices: [], // Start with no MCP services to keep it simple
    managed: true,
    otherConfig: [],
    modelProviderCommented: false,
  };
